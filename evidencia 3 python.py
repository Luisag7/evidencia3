...
try:
    with sqlite3.connect("practica1_SQLlite4.db") as conn:
        c = conn.cursor()
        ...
        #c.execute("CREATE TABLE IF NOT EXISTS venta_detalle (idVenta_Detalle INTEGER PRIMARY KEY AUTOINCREMENT, descripcion TEXT NOT NULL, cantidad_articulo INTEGER NOT NULL, precio_unitario FLOAT NOT NULL, venta INTEGER NOT NULL, FOREIGN KEY(venta) REFERENCES venta(folio));")
...
def registrarArticulo():
    try:
        with sqlite3.connect("practica1_SQLlite4.db") as conn: #1- Puente
            ...
            #valores2 = {"descripcion":descripcion,"cantidad_articulo":cantidad_articulo,"precio_unitario":precio_unitario,"folio":folio}
            mi_cursor.execute("INSERT INTO venta VALUES(null,:descripcion,:cantidad_articulo,:precio_unitario,:Fecha)", valores)
            #mi_cursor.execute("INSERT INTO venta_detalle VALUES(null,:descripcion,:cantidad_articulo,:precio_unitario)", valores2)
...
def consultarArticulo(valor_folio):
    try:
        with sqlite3.connect("practica1_SQLlite4.db") as conn:#1 Crear la conexión
            ...
            print("Clave\tDescripcion\tCantidad\tPrecio\tfecha")
            print("*" * 30)
            if registro:
...
def consultarFecha():
    try:
        with sqlite3.connect("practica1_SQLlite4.db") as conn:#1 Crear la conexión
            ...
            registro = mi_cursor.fetchall()#4 Recuperamos los datos resultantes
            print("Clave\tArticulo\tCantidad\tPrecio\tfecha")
            print("*" * 30)
            if registro:
                for folio, descripcion, cantidad, precio, fecha in registro:
                    print(f"{folio} \t", end="")
...
while True:
    BorrarPantalla()
    print("[1] Registrar una articulo")
    print("[2] Consultar un articulo")
    print("[3] Consultar todos los articulos")
    print("[4] Salir")
    ...
    if respuesta == 1:
        while True:
            ...
            #FechaVenta = datetime.datetime.combine(datetime.date.today(), datetime.datetime.min.time())
            #FechaVenta = datetime.datetime.combine(datetime.date.today(), datetime.datetime.min.time())
            registrarArticulo()
